...
class OpenAIServingResponses(OpenAIServingChat):
                    ...
                    sampling_params = request.to_sampling_params(
                        ...
                        skip_special_tokens=(
                            ...
                            if (
                                processed_messages
                                and any(
                                    tool.type == "function" for tool in request.tools
                                )
    ...
    async def _make_request(
        ...
        """Build chat-completions inputs for the non-Harmony Responses path.

        Errors propagate to ``create_responses``, which converts the expected set
        (``ValueError`` / ``TypeError`` / ``RuntimeError`` / ``jinja2.TemplateError``)
        into proper error responses. Avoid ad-hoc fallback concatenation because it
        silently degrades multimodal/tool/template handling.
        """
    ...
    def _make_response_output_items(
        ...
        # ResponseFunctionToolCall items instead of leaking raw markup as text.
        chat_tools = self._response_tools_to_chat_tools(request)
        if (
            content
            and chat_tools
    ...
    def _response_tools_to_chat_tools(request: ResponsesRequest) -> list[Tool]:
        # Built-in tools (web_search_preview, code_interpreter) flow through the
        # harmony/tool_server path; an empty return is the caller's signal to
        # force tool_choice="none" on the chat request.
        chat_tools = []
        for tool in request.tools:
    ...
    def _construct_input_messages(
        ...
        if prev_response is not None:
            # Add the previous input messages.
            prev_msg = self.msg_store[prev_response.id]
            messages.extend(prev_msg)

            # Replay the previous assistant response as a single chat turn.
            # Responses stores each function call as its own output item, but
            # OpenAI-compatible chat history expects one assistant message with
            # all parallel tool_calls, followed by the caller's tool messages.
            assistant_text_parts = []
            assistant_tool_calls = []
            for output_item in prev_response.output:
                output_type = getattr(output_item, "type", None)
                if output_type is None and hasattr(output_item, "model_dump"):
                    output_type = output_item.model_dump(exclude_none=True).get("type")
                elif isinstance(output_item, dict):
                    output_type = output_item.get("type")

                if output_type == "function_call":
                    tool_msg = self._normalize_response_message_for_chat(output_item)
                    assistant_tool_calls.extend(tool_msg.get("tool_calls", []))
                    continue

                text = self._output_message_text(output_item)
                if text:
